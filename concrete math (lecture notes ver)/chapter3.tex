...
\section{处理取整的一些技巧}
    ...
    技巧三：把一个量写成差分的前缀和再次发挥作用。例如 $\lfloor x\rfloor^2=\sum_{i\ge 1}(i^2-(i-1)^2)[i\le \lfloor x\rfloor]=\sum_{i\ge 1}(i^2-(i-1)^2)[i\le x]$。在使用技巧二和三时，要记得交换求和总是很有用。
...
\end{solution}
\begin{remark}
    $\nabla S(n)=A\ast \mathbf 1$ 实际上是狄利克雷前缀和，可以 $O(n\log\log n)$ 计算。\graffiti{2025 年大家发现了一般序列狄利克雷卷积的 $O(n(\log\log n)^2)$ 算法。参见\href{https://www.cnblogs.com/Elegia/p/18795045/alternative-dirichlet-convolution}{EI 博客}。}
\end{remark}
\begin{solution}[另解]
...
\section{整除分块}
    $\lfloor n/i\rfloor$ 只有 $\le 2\sqrt{n}$ 种不同的取值（要么 $n/i\le \sqrt{n}$，要么 $i\le \sqrt{n}$）。在一个 $\lfloor n/i\rfloor$ 的等值区间内，可以做一些批量的操作（如区间加/求和等）。
    ...
    \end{solution}
    \begin{remark}
        典型的无需预处理也可以快速计算的 $g$ 是 $P(i)r^i$（其中 $P$ 是低次多项式）。
    \end{remark}
...
\begin{solution}
    ...
    \begin{lemma}
        \graffiti{我更愿意称之为「事实」而非「引理」}
         对 $\alpha\in\mathbb R$，当 $n\to\infty$ 时，
        $$
...
\end{solution}
\begin{solution}[更进一步]
    我们可以通过预处理做到更好的复杂度。在先前的例子中，我们已经证明了 $f(n)$ 是约数个数函数 $d(n)$ 的前缀和。对于任何积性函数，我们可以用线性筛筛出其前 $n$ 项，进而求出其前缀和。所以，我们可以用 $O(B)$ 的复杂度预处理所有 $k\le B$ 的 $f(k)$。对于 $k>B$ 的部分，我们还是用整除分块计算。假设 $B>\sqrt{n}$。现在，只有 $\lfloor n/i\rfloor \ge B$ 也就是 $i\le n/B$ 的部分需要使用整除分块，总复杂度为
    $$
...
\section{单位根反演}\graffiti{这个名字应该是 OI 圈生造的。反演在哪？}
    假设存在某个 $\omega_n$ 使得
    $$
    ...
    \begin{exercise}
    对于 $[i\bmod n=j]$，导出一个类似的和式
    \end{exercise}

    \textbf{本原单位根}就满足这个性质。定义如下：
    \begin{definition}[单位根（root of unity）]
        任何满足 $\omega_n^n=1$ 的元素 $\omega_n\in\mathbb F$ 都是域 $\mathbb F$ 中的一个 $n$ 次\textbf{单位根}。如果额外满足 $\forall 0<i<n,\omega_n^i\ne 1$，那么称为 $n$ 次\textbf{本原单位根}。  
    \end{definition}

    下面我们说单位根的时候都指的是本原单位根。

    \begin{exercise}
        对于本原单位根 $\omega_n$，证明 $\omega_n,\omega_n^1,\omega_n^2,\cdots,\omega_n^{n-1},1$ 为方程 $x^n=1$ 的 $n$ 个\textbf{不同}的根。
    \end{exercise}

    \begin{remark}
            目前，你可以认为域是一个可以做加减乘除的集合。常见的域包括有理数 $\mathbb Q$，实数 $\mathbb R$，复数 $\mathbb C$，以及 OI 中最常见的有限域 $\mathbb F_p$（可以认为是 $\bmod\ p$ 下做运算）。需要注意前面几个和最后一个的性质很不相同\graffiti{这是因为它们的\textbf{特征}不同}。
    \end{remark}
    \begin{theorem}[“单位根反演”]
        设 $\omega_n$ 是一个 $n$ 次本原单位根，那么
        $$
        \frac{1}{n}\sum_{j=0}^{n-1}(\omega_n^i)^j=[i\bmod n=0].
        $$
    \end{theorem}
    ...
    \end{proof}
    \begin{example}
        在特征不为 $2$ 的域中，$\pm 1$ 是 $x^2=1$ 的两个根，所以 $-1$ 可以作为 $\omega_2$。于是我们有
        $$
    ...
    \end{solution}
    \begin{remark}
        实际上这是\textbf{离散傅里叶变换}（Discrete Fourier Transform, DFT）的一个特例。离散傅里叶变换 $\mathtt{DFT}_n$ 事实上是在 $\bmod (x^n-1)$ 下做的。换句话说，
    $$
...
\subsection{扩域}
    ...
    在一般情况下，$\omega_k$ 的幂之间的关系由其\textbf{最小多项式} $M(x)$ 决定。假设其次数 $\deg M(x)=m$，那么 $1,\omega_k,\ldots,\omega_k^{m-1}$ 之间线性无关，元素之间的乘法可以视为在 $\bmod\ M(x)$ 下做运算。这部分可以参考域论以及 Galois 理论。
\section{Kummer 定理}
    对于素数 $p$，记 $v_p(n)$ 为 $n$ 的质因子分解中 $p$ 的指数。

    \begin{theorem}[Kummer 定理]
...
\section{补充的 OI 题及解答}
    ...
    \begin{solution}
        ...
        我们讲过的两种推导都需要 $O(k^3)$ 的预处理时间。而通过生成函数\graffiti{虽然我们还没有讲}，我们可以得到一个 $O(k^2)$ 的预处理算法。考虑 EGF
        $$
        \begin{aligned}
        ...
        $$
    \end{solution}
    \begin{example}[类欧几里得算法]
        
    \end{example}
    \begin{solution}
        TODO.
    \end{solution}
    \begin{example}[小猪佩奇学数学]
...
\begin{summary}
    \begin{itemize}
    \item 同余等差数列的结构：$((a+it)\bmod n)_{i=0}^{n-1}$ 形成 $d=\gcd(n,t)$ 个相同的长为 $n/d$ 的排列。
    \item 处理取整的一些技巧：分离整数和小数部分、用新变量和不等式条件替换取整、差分再求和、关于 $n$ 差分
    \item 整除分块：$\lfloor n/i\rfloor$ 的取值很少，用 $\lfloor n/\lfloor n/i\rfloor\rfloor$ 计算当前取值的 $i$ 的右端点
    \item $\sum_{i=1}^n i^{\alpha}\sim\begin{cases}\frac{n^{\alpha+1}}{\alpha+1}&\alpha>-1\\\ln n& \alpha=-1\\O(1)&\alpha<-1\end{cases}$
    \item 单位根反演：$[n\bmod k=0]=\frac{1}{k}\sum_{i=0}^{k-1}\omega_k^{in}$
    \end{itemize}
\end{summary}