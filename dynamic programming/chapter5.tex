...
\end{example}
\begin{solution}
    对于 $i$，相关的区间 $[l,r]$ 为 $l\le i$，$r\ge i$，$L\le r-l+1\le R$ 圈出来的梯形，其中一个点的权值为 $s_r-s_{l-1}$，求最大值。由于最大值可重复贡献，可以将梯形变成两个平行四边形的并。对于平行四边形的询问，相当于计算形如
    $$
    \max_{l\in [L'_i,R'_i]}\max_{r-l+1\le d}s_r-s_{l-1}
    $$
    的问题，其中 $[L'_i,R'_i]$ 是和 $i$ 有关的定长区间，$d$ 是和 $R-L$ 有关的定值。首先可以用单调队列计算所有 $ms_l=\max_{r-l+1\le d}s_r-s_{l-1}$，然后再单调队列计算所有 $\max_{l\in L'_i,R'_i}ms_l$。
\end{solution}
\begin{remark}
    事实上三角形内的结果也是能算的，参见 P6780.
\end{remark}
...
\end{solution}
\begin{solution}[做法三]
    $r$ 从 $1$ 扫描到 $n$，维护每个 $l$ 的答案。假设 $a[i]$ 的上一次出现位置是 $j$，那么 $r$ 从 $i-1$ 移动到 $r$ 的时候，$(j,i]$ 的答案加一，其他位置不变。
\end{solution}
\begin{remark}
    在下一章中我们来处理带修的情况。
\end{remark}
...
\end{example}
\begin{solution}
    在扫描 $r$ 的时候维护每个 $l$ 的答案。对每个 $w$ 维护其对应的 $b$ 的单调栈，则区间加减即可维护答案。
\end{solution}
...
\end{example}
\begin{solution}
    对序列扫描线，则在时间维上需要支持单点加入和删除分治信息。线段树即可。
\end{solution}
\begin{remark}
    这个问题的强制在线需要一些聪明的手段。
\end{remark}
...
\end{example}
\begin{solution}
    先忽略这个裁剪，对序列扫描线，维护一个位置在每个时刻上的值。如果我们可以计算出 $t$ 时刻前最后一次裁剪发生的时刻，那么就容易计算出 $t$ 时刻的真实值了。注意到如果一段时间内 $0$ 与 $c_i$ 都被到达过，则这段时间内的\textbf{极差}必然大于等于 $c_i$。我们在线段树上二分出最大的满足 $[t_0,t]$ 内的极差 $\ge c_i$ 的时刻 $t_0$，则最后一次裁剪发生的时刻即为 $[t_0,t]$ 内的 $\max$ 或者 $\min$。如果 $[0,t]$ 内的极差小于 $c_i$，则说明 $c_i$ 没有被到达过，$[0,t]$ 内的 $\min$ 即为最后一次裁剪发生的时刻。
    
    某个时刻上的修改相当于时间轴上的后缀加，使用支持后缀加、查询区间最值的线段树即可。
\end{solution}
\begin{exercise}[P7560]
\end{exercise}
