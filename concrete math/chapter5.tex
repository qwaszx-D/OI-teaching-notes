...
\end{frame}
\begin{frame}{商集与除法原理}
    在很多时候，问题中会显式地或隐式地有“...视为同一种方案”这种表述。有时，我们可以先无视“视为同一种方案”，然后再除掉一个因子来得到答案。有时，我们则不能这样做。这往往是计数中难以理解的部分。
    
    除法原理何时能使用？不能使用除法原理又该怎么办？我们将用等价关系、等价类和商集的概念来说明。
\end{frame}
\begin{frame}{关系和等价关系}
...
\begin{frame}{等价类，商集}
    ...
    常见的做法是在每个等价类内选一个\textbf{代表元}来表示 $S/R$ 的一个元素。例如，考虑 $\mathbb Z$ 上的等价关系 $\equiv\pmod m$，我们用 $\bar x=x+\mathbb mZ$ 来代表所有 $\equiv x\pmod m$ 的元素形成的等价类。那么商集的元素就是所有的 $\bar i$，其中 $0\le i<m$。一般来说，对于 $x\in S$，我们用 $\bar x$ 代表 $S/R$ 中包含 $x$ 的那个等价类。
\end{frame}
\begin{frame}{除法原理}
...
\begin{frame}{经典组合模型}
    我们接下来会给出很多经典的组合计数模型。熟悉这些模型是必要的：
    \begin{itemize}
        \item 第一，任何大的未知问题最后都要分解成小的已知问题
        \item 第二，我们必须要在实际的问题中使用我们先去学到的技巧
        \item 第三，我们已经练习了太多代数推导，该练习一下组合直观了
    \end{itemize}
\end{frame}
\begin{frame}{球与盒子}
    洛谷 P5824 十二重计数法（由于本题实际需要 FFT，我们只列式子）。$n$ 个球装进 $m$ 个盒子里，模型分别为
    \begin{enumerate}[I]
...
\begin{frame}{生成函数}
    ...
    然而，考虑 $m$ 的 GF 则寸步难行。所以，我们总是应该关于“待分配对象”建立生成函数。在球盒模型中，待分配对象是球，而用来分配的容器则是盒子。
\end{frame}
\begin{frame}{标号（一）}
    有区别/可区分/有标号/互不相同都指向相同的概念。一般来说，有标号是最通用和方便的说法。

    我们已经在前面看到，球与盒子都有标号时，可以逐球考虑/逐盒子考虑来计数。

    在球有标号、盒子无标号时，盒子的任意排列被视为同一种方案（等价类）。此时，常见的处理方法是\textbf{强制}一个顺序（“选代表元”），或者使用\textbf{除法原理}。
        \begin{itemize}
...
\begin{frame}{X, XI 和 XII}
    ...
    对于 X，这是一个拆分数类型的问题。此类问题的直观理解如下：把每个球画成一个点，把一个盒子里装的球画成一行的点，不同盒子画在不同行并对齐，强制盒子里装的球数从上往下不降。这样得到的点阵被称为 \textbf{Ferrers} 图。例如，右图就是 2, 3, 5, 5 的 Ferrers 图。我们将问题转化为一定限制下的 Ferrers 图计数。
...
\begin{frame}
    ...
    两个生成函数看起来有微妙的不同，这是 $f_0$ 导致的。事实上，上面那个生成函数才是完全正确的，下面这个会导致 $f_0=m/(m-1)$。不过，把上面那个分式化成真分式后就会发现其他项都是相同的。这两个分式对应的线性递推和做法二是相同的。
\end{frame}
\begin{frame}
    另外，也可以直接展开生成函数：
    $$
