...
\end{frame}
\begin{frame}{域}
    目前，你可以认为域是一个可以做加减乘除的集合。常见的域包括有理数 $\mathbb Q$，实数 $\mathbb R$，复数 $\mathbb C$，以及 OI 中最常见的有限域 $\mathbb F_p$（可以认为是 $\bmod\ p$ 下做运算）。需要注意前面几个和最后一个的性质很不相同\footnote{这是因为它们的\textbf{特征}不同}。
\end{frame}
\begin{frame}{证明}
    考虑
    $$
...
\end{frame}
\begin{frame}
    在一般情况下，$\omega_k$ 的幂之间的关系由其\textbf{最小多项式} $M(x)$ 决定。假设其次数 $\deg M(x)=m$，那么 $1,\omega_k,\ldots,\omega_k^{m-1}$ 之间线性无关，元素之间的乘法可以视为在 $\bmod\ M(x)$ 下做运算。
