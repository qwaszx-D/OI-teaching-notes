...
\begin{solution}
    ...
    注意如果取模后发生变化，那么这个数至少会减半。因此每个数在两次单点修改之间只会被有效修改 $O(\log V)$ 次。
\end{solution}
\begin{remark}
...
\begin{example}
    查询最短的按位或和 $\ge x$ 的区间长度，$x$ 每次不同。
\end{example}
\begin{solution}
    对每个 $l$，$f(l,r)$ 关于右端点 $r$ 形成至多 $\log V$ 个等值连续段，每个连续段只有第一个位置可能是最短的。所以只需要计算出所有这样的 $O(n\log V)$ 个区间，然后简单查询即可。

    不难发现这样的区间就是上面的暴力中 if 条件成立的那些 $[j,i]$。
\end{solution}
...
\end{example}
\begin{solution}
    查询点 $u$ 时，如果 $\deg_u\le \sqrt{2m}$，那么暴力就是 $O(\sqrt{m})$ 的。对于 $\deg>\sqrt{2m}$ 的 $O(\sqrt{m})$ 个点，可以单独维护。那么在修改 $u$ 时，需要更新 $u$ 的那些 $\deg>\sqrt{2m}$ 的邻居的答案，这可以通过在建图时就计算出这些邻居的列表来完成。
\end{solution}
...
\end{example}
\begin{solution}
    首先我们有一个 $\sum_u\sum_{v\in N(u)}\deg_v=\sum_v \deg_v^2$ 的做法：枚举一个点 $u$，标记其所有邻居；然后枚举 $u$ 的邻居 $v$，再枚举 $v$ 的邻居 $w$，如果 $w$ 被标记了，那么 $u,v,w$ 就构成一个三元环。此外，每个三元环在这个算法中都会被枚举到三次。
    
    不过我们可以做得更聪明一点：如果可以把枚举 $v$ 的邻居的代价从 $\deg_v$ 降低至 $\min(\deg_v,\sqrt{2m})$，那么总复杂度就可以降到 $O(m\sqrt{m})$。这可以通过强制 $\deg_u\le \deg_v\le \deg_w$ 完成。或者说，我们把所有点按照度数排序，然后把每条无向边定向，使得每条边都是靠前的点连向靠后的点。这样，当 $\deg_v>\sqrt{2m}$ 时，对应的 $w$ 必须满足 $\deg_w>\sqrt{2m}$，而这样的点数不超过 $\sqrt{2m}$。在这个算法中，每个三元环恰好被枚举到一次。
\end{solution}
\begin{remark}
    可以用矩阵乘法做到 $O(n^\omega)$。
\end{remark}
...
\begin{example}
    多次询问值为 $x$ 的位置集合和值为 $y$ 的位置集合之间的最小距离。    
\end{example}
\begin{solution}
    按照每个值的出现次数是否 $>\sqrt{n}$ 分成两类。如果 $x,y$ 出现次数都不超过 $\sqrt{n}$，那么可以归并计算。否则，通过预处理完成。

    对每个出现次数 $>\sqrt{n}$ 的值 $x$，对所有 $y$ 计算 $ans_{x,y}$。这可以对每个 $x$ 正反扫两遍序列得到。

    可以离线逐 $x$ 计算来做到线性空间。
\end{solution}
\begin{remark}
    另一个做法是只预处理出现次数 $>\sqrt{n}$ 的 $y$。当 $x,y$ 有一个出现次数不超过 $\sqrt{n}$ 时，暴力枚举那个值的所有出现，在另一个值的出现数组中二分即可。这样空间是线性的，但是时间带 $\log$。可以使用 $B$ 叉树将二分变成 $O(1)$，但是空间会增大。不知道在线能否做到 $n\mathrm{polylog}(n)$ 的空间。

    此外，这个问题似乎被称为 color-distance oracle。
\end{remark}
\begin{exercise}
    额外把位置集合限制到区间 $[l,r]$ 内。
\end{exercise}
...
\end{example}
\begin{solution}
    模拟二进制竖式加法。从低位到高位依次计算每一位的结果和进位。每一位的结果可以通过异或计算得到，进位可以通过与和或计算得到。
\end{solution}
\begin{example}[并行加法器]
    有 $n$ 个数，需要统计每一位上总共有多少个 $1$。
\end{example}
\begin{solution}
    分治计算每一位的答案。那么合并的时候需要做 $w$ 个 $\log n$ bit 的加法，可以将加法电路并行来做到 $O(\log n)$ 而不是 $O(w)$。具体来说，常规的每位加法维护了 $w\times \log n$ 的 01 数组，这里则是维护转置后 $\log n\times w$ 的 01 数组。
\end{solution}
