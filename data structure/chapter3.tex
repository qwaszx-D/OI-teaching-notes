...
\begin{remark}
    事实上，这里有一个下界。[Yao85] 证明了代数运算次数的下界满足 $t_q\log(1+t_u/t_q)=\Omega(\log n)$，[Pǎtraşcu, Demaine '04] 及后续工作则证明了内存访问次数的下界也是 $\Omega\left(\frac{\delta}{w}\log n\right)$，其中 $\delta$ 是每个元素的大小，$w$ 是机器字长。
\end{remark}
